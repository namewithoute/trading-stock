...
func (p *Projector) Rebuild(ctx context.Context) error {
	p.logger.Info("[ Projector ] Starting incremental catch-up rebuild from Kafka...")

	rebuildReader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:        p.brokers,
		Topic:          KafkaTopicAccountEvents,
		GroupID:        "account-projector",
		MinBytes:       1,
		MaxBytes:       10e6,
		MaxWait:        1 * time.Second,
	...
	for {
		...
		if err != nil {
			...
			return fmt.Errorf("rebuild fetch from Kafka: %w", err)
		}

		if err := p.project(ctx, msg); err != nil {
			p.logger.Warn("[ Projector ] Skipping event during incremental Rebuild",
		...
		if err := rebuildReader.CommitMessages(ctx, msg); err != nil {
			return fmt.Errorf("rebuild commit offset: %w", err)
		}

		eventCount++
	}

	p.logger.Info("[ Projector ] Incremental catch-up rebuild complete",
...
func (p *Projector) project(ctx context.Context, msg kafka.Message) error {
	...
	if err := infraEvents.DecodeKafkaPayload(msg.Value, &descriptor); err != nil {
